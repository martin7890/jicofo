...
public class ChatRoomImpl extends AbstractChatRoom {
	 ...
	 */
	private CopyOnWriteArrayList<ChatRoomMemberPresenceListener> listeners = new CopyOnWriteArrayList<ChatRoomMemberPresenceListener>();

	/**
	 * Local user role listeners.
	 */
	private CopyOnWriteArrayList<ChatRoomLocalUserRoleListener> localUserRoleListeners = new CopyOnWriteArrayList<ChatRoomLocalUserRoleListener>();
	 ...
	 *
	 * @param parentChatOperationSet
	 *            parent multi user chat operation set.
	 * @param roomName
	 *            the name of the chat room that will be handled by new
	 *            <tt>ChatRoomImpl</tt>instance.
	 */
	public ChatRoomImpl(OperationSetMultiUserChatImpl parentChatOperationSet,
		...
		muc = new MultiUserChat(parentChatOperationSet.getConnection(),
				roomName);
	...
	public void joinAs(String nickname) throws OperationFailedException {
		try {
			...
			/*
			 * Iterator<FormField> fields = config.getFields(); while
			 * (fields.hasNext()) { FormField field = fields.next();
			...
			throw new OperationFailedException("Failed to join the room",
	...
	public ChatRoomMemberRole getUserRole() {
		if (this.role == null) {
			Occupant o = muc.getOccupant(muc.getRoom() + "/"
					+ muc.getNickname());

			if (o == null)
				return ChatRoomMemberRole.GUEST;
			else
				this.role = ChatRoomJabberImpl.smackRoleToScRole(o.getRole(),
						o.getAffiliation());
		}
	...
	private void resetRoleForParticipant(String participant) {
		if (participant.endsWith("/" + myNickName)) {
			...
			if (member != null) {
				...
				logger.error("Role reset for: " + participant
						+ " who does not exist");
			}
		}
	}
	 ...
	 *
	 * @param previousRole
	 *            the previous role that local user had
	 * @param newRole
	...
	private void fireLocalUserRoleEvent(ChatRoomMemberRole previousRole,
		...
		ChatRoomLocalUserRoleChangeEvent evt = new ChatRoomLocalUserRoleChangeEvent(
	 ...
	 *
	 * @param role
	 *            the new role to be set for the local user
	 * @param isInitial
	 *            if <tt>true</tt> this is initial role set.
	 */
	...
	public void grantOwnership(String address) {
		...
		if (reply.getType() != IQ.Type.RESULT) {
			...
			throw new RuntimeException("Failed to grant owner: "
					+ reply.getError());
		}
	}
	...
	private void notifyParticipantJoined(ChatMemberImpl member) {
		ChatRoomMemberPresenceChangeEvent event = new ChatRoomMemberPresenceChangeEvent(
				this, member, ChatRoomMemberPresenceChangeEvent.MEMBER_JOINED,
				null);
	...
	private void notifyParticipantLeft(ChatMemberImpl member) {
		ChatRoomMemberPresenceChangeEvent event = new ChatRoomMemberPresenceChangeEvent(
				this, member, ChatRoomMemberPresenceChangeEvent.MEMBER_LEFT,
				null);
		for (ChatRoomMemberPresenceListener l : listeners) {
			l.memberPresenceChanged(event);
		}
	...
	private void notifyParticipantKicked(ChatMemberImpl member) {
		ChatRoomMemberPresenceChangeEvent event = new ChatRoomMemberPresenceChangeEvent(
				this, member, ChatRoomMemberPresenceChangeEvent.MEMBER_KICKED,
				null);
	...
	public void sendPresenceExtension(PacketExtension extension) {
		...
		// Remove old
		PacketExtension old = lastPresenceSent.getExtension(
	...
	class MemberListener implements ParticipantStatusListener {
		...
		public void left(String participant) {
			...
			if (member != null) {
				...
				logger.warn("Member left event for non-existing participant: "
						+ participant);
			}
		}
		...
		public void kicked(String participant, String s2, String s3) {
			...
			if (member == null) {
				logger.error("Kicked participant does not exist: "
						+ participant);
				return;
			}
	...
	class ParticipantListener implements PacketListener {
		...
		private void processOwnPresence(Presence presence) {
			...
			if (mucUser != null) {
				...
				ChatRoomMemberRole jitsiRole = ChatRoomJabberImpl
						.smackRoleToScRole(role, affiliation);
