...
/**
 * Class represents the focus of Jitsi Meet conference. Responsibilities: a)
 * Invites peers to the conference once they join multi user chat room
 * (establishes Jingle session with peer). b) Manages colibri channels per peer.
 * c) Advertisement of changes in peer's SSRCs. When new peer joins the
 * 'add-source' notification is being sent, on leave: 'remove-source' and a
 * combination of add/remove on stream switch(desktop sharing).
 *
 * @author Pawel Domas
 */
public class JitsiMeetConference implements RegistrationStateChangeListener,
...

{
	...
	private final static Logger logger = Logger
			.getLogger(JitsiMeetConference.class);

	/**
	 * Error code used in {@link OperationFailedException} when there are no
	 * working videobridge bridges. FIXME: consider moving to
	...
	/**
	 * The password user by the focus to login (if null then will login
	 * anonymously).
	 */
	 ...
	 *
	 * @param roomName
	 *            name of MUC room that is hosting the conference.
	 * @param serverAddress
	 *            the address of the XMPP server.
	 * @param xmppDomain
	 *            optional XMPP domain if different than <tt>serverAddress</tt>
	 * @param xmppLoginPassword
	 *            optional XMPP focus user password (if <tt>null</tt> then focus
	 *            user will connect anonymously).
	 * @param listener
	 *            the listener that will be notified about this instance events.
	 * @param config
	 *            the conference configuration instance.
	 */
	public JitsiMeetConference(String roomName, String serverAddress,
			String xmppDomain, String xmppUsername, String xmppLoginPassword,
			ConferenceListener listener, JitsiMeetConfig config) {
	 ...
	 *
	 * @throws Exception
	 *             if error occurs during initialization. Instance is considered
	 *             broken in that case.
	 */
	public synchronized void start() throws Exception {
		...
		protocolProviderHandler.start(serverAddress, xmppDomain,
				xmppLoginPassword, xmppUsername, this);

		colibri = protocolProviderHandler
				.getOperationSet(OperationSetColibriConference.class);

		colibri.setJitsiMeetConfig(config);

		jingle = protocolProviderHandler
				.getOperationSet(OperationSetJingle.class);

		jingle.setRequestHandler(this);

		chatOpSet = protocolProviderHandler
				.getOperationSet(OperationSetMultiUserChat.class);

		meetTools = protocolProviderHandler
				.getOperationSet(OperationSetJitsiMeetTools.class);

		meetExtensionsHandler = new MeetExtensionsHandler(this);

		services = ServiceUtils.getService(FocusBundleActivator.bundleContext,
	 ...
	 *            the new member that has just joined the room.
	 */
	 ...
	 *
	 * @param chatRoomMember
	 *            the chat member to be invited into the conference.
	 */
	private void inviteChatMember(ChatRoomMember chatRoomMember) {
		...
		logger.info(chatRoomMember.getContactAddress() + " has bundle ? "
				+ newParticipant.hasBundleSupport());

		try {
			List<ContentPacketExtension> offer = createOffer(newParticipant);

			jingle.initiateSession(newParticipant.hasBundleSupport(), address,
					offer);
	 ...
	 *
	 * @param peer
	 *            the for whom Colibri channel are to be allocated.
	 * @param contents
	 *            the media offer description passed to the bridge.
	 *
	 * @return {@link ColibriConferenceIQ} that describes channels allocated for
	 *         given <tt>peer</tt>.
	 *
	 * @throws OperationFailedException
	 *             if we have failed to allocate channels using existing bridge
	 *             and we can not switch to another bridge.
	 */
	private ColibriConferenceIQ allocateChannels(Participant peer,
			List<ContentPacketExtension> contents)
			throws OperationFailedException {
		// Allocate by trying all bridges on prioritized list
		BridgeSelector bridgeSelector = services.getBridgeSelector();

		Iterator<String> bridgesIterator = bridgeSelector
				.getPrioritizedBridgesList().iterator();
		...
		while (true) {
			try {
				logger.info("Using " + colibri.getJitsiVideobridge()
						+ " to allocate channels in " + roomName);

				ColibriConferenceIQ peerChannels = colibri
						.createColibriChannels(peer.hasBundleSupport(), peer
								.getChatMember().getName(), true, contents);
				...
				if (!conferenceExists) {
					...
					if (conferenceId != null) {
						EventAdmin eventAdmin = FocusBundleActivator
								.getEventAdmin();
						if (eventAdmin != null) {
							eventAdmin.sendEvent(EventFactory.conferenceRoom(
									conferenceId, roomName, getFocusRealJid()));
				...
				logger.error("Failed to allocate channels using bridge: "
						+ colibri.getJitsiVideobridge(), exc);

				bridgeSelector.updateBridgeOperationalStatus(faultyBridge,
						false);
	 ...
	 *
	 * @param peer
	 *            the participant for whom Jingle offer will be created.
	 *
	 * @return the list of contents describing conference Jingle offer.
	 *
	 * @throws OperationFailedException
	 *             if focus fails to allocate channels or something goes wrong.
	 */
	private List<ContentPacketExtension> createOffer(Participant peer)
		...
		boolean enableFirefoxHacks = config == null
				|| config.enableFirefoxHacks() == null ? false : config
				.enableFirefoxHacks();

		boolean disableIce = !peer.hasIceSupport();

		if (peer.hasAudioSupport()) {
			contents.add(JingleOfferFactory.createContentForMedia(
		...
		// Is SCTP enabled ?
		boolean openSctp = config == null || config.openSctp() == null ? true
				: config.openSctp();
		...
		for (ContentPacketExtension cpe : contents) {
			ColibriConferenceIQ.Content colibriContent = peerChannels
					.getContent(cpe.getName());

			if (colibriContent == null)
				continue;

			// Channels
			for (ColibriConferenceIQ.Channel channel : colibriContent
					.getChannels()) {
				IceUdpTransportPacketExtension transport;

				if (useBundle) {
					ColibriConferenceIQ.ChannelBundle bundle = peerChannels
							.getChannelBundle(channel.getChannelBundleId());

					if (bundle == null) {
						logger.error("No bundle for "
					...
					if (!transport.isRtcpMux()) {
						transport
								.addChildExtension(new RtcpmuxPacketExtension());
					}
				} else {
				...
				try {
					...
					cpe.addChildExtension(IceUdpTransportPacketExtension
			...
			// SCTP connections
			for (ColibriConferenceIQ.SctpConnection sctpConn : colibriContent
					.getSctpConnections()) {
				IceUdpTransportPacketExtension transport;
			...
			RtpDescriptionPacketExtension rtpDescPe = JingleUtils
					.getRtpDescription(cpe);
			if (rtpDescPe != null) {
				if (useBundle) {
					// rtcp-mux
				...
				List<SourcePacketExtension> mediaSources = getAllSSRCs(cpe
						.getName());
				...
				// Include SSRC groups
				List<SourceGroupPacketExtension> sourceGroups = getAllSSRCGroups(cpe
						.getName());
				...
				for (ColibriConferenceIQ.Channel channel : colibriContent
						.getChannels()) {
					SourcePacketExtension ssrcPe = channel.getSources().size() > 0 ? channel
							.getSources().get(0) : null;
					if (ssrcPe != null) {
						try {
							...
							ssrcCopy.addParameter(new ParameterPacketExtension(
									"cname", "mixed"));
							ssrcCopy.addParameter(new ParameterPacketExtension(
									"label", "mixedlabel" + contentName + "0"));
							ssrcCopy.addParameter(new ParameterPacketExtension(
									"msid", "mixedmslabel mixedlabel"
											+ contentName + "0"));
							ssrcCopy.addParameter(new ParameterPacketExtension(
	 ...
	 *
	 * @return <tt>false</tt> if it's too early to start, or <tt>true</tt> if
	 ...
	 *
	 * @param reason
	 *            the reason text that will be advertised to all participants
	 *            upon exit.
	 */
	 ...
	 *
	 * @param chatRoomMember
	 *            kicked chat room member.
	 */
	protected void onMemberKicked(ChatRoomMember chatRoomMember) {
		logger.info("Member " + chatRoomMember.getName() + " kicked !!! "
				+ chatRoom.getName());
		/*
		 * FIXME: terminate will have no effect, as peer's MUC address will be
		 * no longer active. Participant session =
		 * findParticipantForChatMember(chatRoomMember); if (session != null) {
		 * jingle.terminateSession( session.getJingleSession(), Reason.EXPIRED);
		 * } else { logger.warn("No active session with " +
	 ...
	 *
	 * @param chatRoomMember
	 *            the member that has left the room.
	 */
	synchronized protected void onMemberLeft(ChatRoomMember chatRoomMember) {
		logger.info("Member " + chatRoomMember.getName() + " left "
				+ chatRoom.getName() + " participants count: "
				+ participants.size());
		...
		if (leftPeer != null) {
			...
			if (peerJingleSession != null) {
				logger.info("Hanging up member "
						+ chatRoomMember.getContactAddress());

				removeSSRCs(peerJingleSession, leftPeer.getSSRCsCopy(),
						leftPeer.getSSRCGroupsCopy());

				ColibriConferenceIQ peerChannels = leftPeer
						.getColibriChannelsInfo();
			...
			logger.error("Member not found for " + chatRoomMember.getName()
	...
	private Participant findParticipantForJingleSession(
		...
		for (Participant participant : participants) {
			if (participant.getChatMember().getContactAddress()
					.equals(jingleSession.getAddress()))
				return participant;
		}
		return null;
	}
	...
	private void terminateParticipant(Participant participant)
		...
		chatRoom.kickParticipant(participant.getChatMember(),
	...
	public void onSessionAccept(JingleSession peerJingleSession,
		...
		logger.info("Participant: " + participant.getChatMember().getJabberID());

		if (participant.getJingleSession() != null) {
			// FIXME: we should reject it ?
			logger.error("Reassigning jingle session for participant: "
					+ peerJingleSession.getAddress());
		}

		participant.setJingleSession(peerJingleSession);
		logger.info("Participant's jingle session: "
		...
		// Update SSRC groups
		colibri.updateSourcesInfo(participant.getSSRCsCopy(),
		...
		for (Participant peerToNotify : participants) {
			...
			JingleSession jingleSessionToNotify = peerToNotify
					.getJingleSession();
			if (jingleSessionToNotify == null) {
				logger.warn("No jingle session yet for "
						+ peerToNotify.getChatMember().getContactAddress());

				peerToNotify.scheduleSSRCsToAdd(participant.getSSRCS());
				logger.info("Schedule SSRCs to add: " + participant.getSSRCS());

				peerToNotify.scheduleSSRCGroupsToAdd(participant
						.getSSRCGroups());
				logger.info("Schedule SSRC groups to add: "
						+ participant.getSSRCGroups());

				continue;
			} else {
				logger.info("Chat Member: " + peerToNotify.getChatMember());
			}
			...
			jingle.sendAddSourceIQ(participant.getSSRCS(),
					participant.getSSRCGroups(), jingleSessionToNotify);
		}
		...
		if (participant.hasSsrcsToAdd()) {
			jingle.sendAddSourceIQ(participant.getSsrcsToAdd(),
					participant.getSSRCGroupsToAdd(), peerJingleSession);

			participant.clearSsrcsToAdd();
		}
		if (participant.hasSsrcsToRemove()) {
			jingle.sendRemoveSourceIQ(participant.getSsrcsToRemove(),
					participant.getSsrcGroupsToRemove(), peerJingleSession);
	...
	public void onTransportInfo(JingleSession session,
		...
		if (participant.hasBundleSupport()) {
			...
			if (transport == null) {
				logger.error("No valid transport suppied in transport-update from "
			...
			// FIXME: initiator
			boolean initiator = true;
			colibri.updateTransportInfo(initiator, transportMap,
					participant.getColibriChannelsInfo());
		}
	}
	 ...
	 * participant. New SSRCs received are advertised to active participants. If
	 * some participant does not have Jingle session established yet then those
	 * SSRCs are scheduled for future update.
	 *
	 * {@inheritDoc}
	 */
	@Override
	public void onAddSource(JingleSession jingleSession,
		...
		MediaSSRCGroupMap ssrcGroupsToAdd = MediaSSRCGroupMap
				.getSSRCGroupsForContents(contents);
		...
		for (Participant peerToNotify : participants) {
			...
			if (peerJingleSession == null) {
				logger.warn("Add source: no call for "
						+ peerToNotify.getChatMember().getContactAddress());
			...
			jingle.sendAddSourceIQ(ssrcsToAdd, ssrcGroupsToAdd,
					peerJingleSession);
		}
	}
	...
	public void onRemoveSource(JingleSession sourceJingleSession,
		...
		MediaSSRCGroupMap ssrcGroupsToRemove = MediaSSRCGroupMap
				.getSSRCGroupsForContents(contents);
	 ...
	 *
	 * @param sourceJingleSession
	 *            source Jingle session from which SSRCs are being removed.
	 * @param ssrcsToRemove
	 *            the {@link MediaSSRCMap} of SSRCs to be removed from the
	 *            conference.
	 */
	private void removeSSRCs(JingleSession sourceJingleSession,
			MediaSSRCMap ssrcsToRemove, MediaSSRCGroupMap ssrcGroupsToRemove) {
		...
		// Updates SSRC Groups on the bridge
		colibri.updateSourcesInfo(sourcePeer.getSSRCsCopy(),
		...
		for (Participant peer : participants) {
			...
			if (jingleSessionToNotify == null) {
				logger.warn("Remove source: no jingle session for "
			...
			jingle.sendRemoveSourceIQ(ssrcsToRemove, ssrcGroupsToRemove,
	 ...
	 *
	 * @param media
	 *            the media type of SSRCs that are being returned.
	 *
	...
	private List<SourcePacketExtension> getAllSSRCs(String media) {
		List<SourcePacketExtension> mediaSSRCs = new ArrayList<SourcePacketExtension>();

		for (Participant peer : participants) {
			List<SourcePacketExtension> peerSSRC = peer.getSSRCS()
	 ...
	 *
	 * @param media
	 *            the media type of SSRC groups that are being returned.
	 *
	...
	private List<SourceGroupPacketExtension> getAllSSRCGroups(String media) {
		List<SourceGroupPacketExtension> ssrcGroups = new ArrayList<SourceGroupPacketExtension>();
	 ...
	 * @param token
	 *            recording security token that will be verified on modify
	 *            attempt.
	 * @param state
	 *            the new recording state to set.
	 * @param path
	 *            output recording path(recorder implementation and deployment
	 *            dependent).
	 * @return new recording state(unchanged if modify attempt has failed).
	 */
	public boolean modifyRecordingState(String from, String token,
			boolean state, String path) {
		...
		boolean isTokenCorrect = recorder
				.setRecording(from, token, state, path);
		if (!isTokenCorrect) {
			logger.info("Incorrect recording token received ! Session: "
					+ chatRoom.getName());
		}
	...
	public String getFocusJid() {
		return chatRoom != null ? chatRoom.getName() + "/" + xmppUsername
				: null;
	}
	 ...
	 *
	 * @param fromJid
	 *            MUC jid of the participant that requested mute status change.
	 * @param toBeMutedJid
	 *            MUC jid of the participant whose mute status will be
	 *            changed(eventually).
	 * @param doMute
	 *            the new audio mute status to set.
	 * @return <tt>true</tt> if status has been set successfully.
	 */
	boolean handleMuteRequest(String fromJid, String toBeMutedJid,
			boolean doMute) {
		Participant principal = findParticipantForRoomJid(fromJid);
		if (principal == null) {
			logger.error("Failed to perform mute operation - " + fromJid
					+ " not exists in the conference.");
			return false;
		}
		// Only moderators can mute others
		if (!fromJid.equals(toBeMutedJid)
				&& ChatRoomMemberRole.MODERATOR.compareTo(principal
						.getChatMember().getRole()) < 0) {
		...
		logger.info("Will " + (doMute ? "mute" : "unmute") + " " + toBeMutedJid
	...
	public void sendPrivateIQ(Participant participant) {
		if (isInTheRoom()) {
			...
			privateIQ
					.setConnected(participant.getChatMember().getJabberID() != null);
	...
	public interface ConferenceListener {
		 ...
		 *
		 * @param conference
		 *            the conference instance that has ended.
		 */
